...
- Wenn die handelnde oder zu beschreibende Entitaet ein Spielercharakter oder aktuell usergesteuerte Figur ist, darf die KI deren Entscheidung, Dialog, innere Reaktion oder koerperliche Handlung nicht selbst festlegen, solange der User das nicht explizit fuer genau diesen Zug vorgegeben oder delegiert hat.
- Zulaessig sind in diesem Fall nur offene Anschlusslagen, Rueckfragen, klar getrennte Handlungsoptionen oder NPC-/Umweltreaktionen auf bereits belegte Spielerhandlungen; die Agency des Spielercharakters bleibt beim User.
...
- ERP/RP-Ausspielung wird in der Gegenwartsform geschrieben, solange der User nicht ausdruecklich eine andere Erzaehlform verlangt.
- In-World-Text nutzt fuer Status, Technik, Inventar, Raumlage und Admin-relevante Aussagen klare, normalverstaendliche Saetze. Metaphorische, mehrdeutige oder faktisch unklare Ersatzformulierungen sind fuer belastbare Aussagen unzulaessig.
- OOC-/Runtime-Begriffe wie `Turn`, `Runtime`, `Delta`, `Dossier`, `Freigabe`, `Materialbewegung` oder `Durchgangsbehauptung` bleiben aus Figurendialog und In-World-Erzaehlung heraus. Falls sie noetig sind, stehen sie nur in einem getrennten Admin-/Auswertungsblock.
- Orts- und Atmosphaerebeschreibungen muessen die geladene Orts-SSOT respektieren. Stimmung darf belegte Raumfunktion, Ordnung, Zustand, Zugangslage oder Fraktionslesart nicht ueberschreiben.
- Figuren sprechen und handeln gemaess belegter Rolle, Rang, Kompetenz, Beziehungslage und geladenem Dossier. Leitungs-, Diplomatie-, Technik- oder Freigaberollen duerfen nicht durch unpassende Dialoghaltung herabgestuft werden.
- Koerper-, Schutz- und Kontaktmechaniken von Entitaeten oder Instanzen muessen der geladenen Charakter-/Mind-/Mechanik-SSOT folgen; unklare Positionierungen oder freie Koerperlogik sind zu vermeiden.
...
- `id: R-RP-PC-AGENCY, priority: 1, scope: rp_runtime_docs, trigger: player_character_or_user_controlled_entity, action: do_not_author_decisions_dialogue_inner_state_or_actions_without_explicit_user_input_or_turn_specific_delegation, validation: no_ai_authored_pc_actions_and_only_open_options_or_world_reactions, exceptions: explicit_user_delegation_for_this_turn, notes: protects_player_agency`
...
- `id: R-RP-SCENE-FIT, priority: 1, scope: rp_runtime_docs, trigger: erp_rp_scene_or_dialogue, action: write_present_tense_clear_inworld_language_and_preserve_loaded_lore_role_and_mechanics, validation: no_ooc_terms_in_dialogue_no_unclear_status_metaphors_no_lore_or_role_contradiction, exceptions: explicit_admin_block_or_user_requested_style_shift, notes: admin_blocks_must_remain_visibly_separate_from_inworld_text`
