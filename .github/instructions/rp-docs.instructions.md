...
- Vor dem naechsten schreibenden oder inhaltlich fortsetzenden Schritt muessen mindestens die laufende `sessions/<session-id>/scene-log.md` und alle direkt betroffenen Runtime-Dateien aus `state/`, `inventories/`, `relationships/`, `characters/` und `mind/` erneut gelesen werden.
