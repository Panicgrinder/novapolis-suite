Stand: 2026-01-08 14:18 – Policy-Refresh: keine .ps1-Wrapper mehr; Beispiele auf Python-Wrapper umgestellt
...
Kanonische Prüfabläufe (Python)
---
### Empfohlen (Wrapper, Copilot/GPT)

```powershell
& .\.venv\Scripts\python.exe scripts\run_pytest_coverage.py
```
...
- Copilot/GPT startet komplexe/mehrschrittige Abläufe nicht als Inline `-Command`, sondern bevorzugt über Python-Wrapper (z. B. `& .\.venv\Scripts\python.exe scripts\<script>.py`). Die nachfolgenden Inline-Beispiele sind dokumentarisch und für manuelle Human-Runs gedacht; Inline `-Command` bleibt nur für echte Einzeiler zulässig.
...
#### Format Postflight (erfolgreich)
   - Meta: Modus=Postflight, Modell=<GPT-5|GPT-5 Codex|GPT-5 mini>, Arbeitsverzeichnis=<Pfad>, RepoRoot=<Pfad>, PSScriptRoot=<Pfad>, PSVersion=<x.y.z>, Aufruf=<Command, z. B. & .\\.venv\\Scripts\\python.exe scripts\\run_checks_and_report.py --scope full>, SHA256=<Hash.der.Skriptdatei>, STOP-Gate=<aktiv/deaktiv>, Wrapper-Policy=<erfüllt/verletzt>, Wrapper-Guards=<GuardA:PASS|GuardB:PASS>, Quellen=<.github/copilot-instructions.md;README.md;...>, Aktion=<Kurzbeschreibung>
     ...
     - Todos: offen=<Anzahl>, BeispielFix=<Kurzbeschreibung>, ReRun=<Testname>, Fällig=<Datum/Zeit>
       - Beispiel (Dummy):
          ...
          ```
         Meta: Modus=Postflight, Modell=GPT-5 Codex, Arbeitsverzeichnis=F:\VS Code Workspace\Main, RepoRoot=F:\VS Code Workspace\Main, PSScriptRoot=F:\VS Code Workspace\Main\scripts, PSVersion=7.4.1, Aufruf=& .\\.venv\\Scripts\\python.exe scripts\\run_checks_and_report.py --scope full, SHA256=..., STOP-Gate=aktiv, Wrapper-Policy=erfüllt, Wrapper-Guards=PfadCheck:PASS|StopGate:PASS, Quellen=.github/copilot-instructions.md;README.md, Aktion=Checks-Wrapper ausgeführt
...
### aktuell vergebene Regel-IDs
   - ID R-WRAP: Wrapper-Policy - Skripte und Mehrschritt-Prozesse dürfen ausschließlich über Python-Wrapper (z. B. `& .\.venv\Scripts\python.exe scripts\<script>.py`) ausgeführt werden. Inline `pwsh -Command` ist nur für echte Einzeiler erlaubt.
...
- R-WRAP Schwelle: Ein Einzeiler umfasst max. einen Prozessaufruf und höchstens einen Pipe-Schritt; darüber Python-Wrapper (unter `scripts/`) anlegen und darüber ausführen.
