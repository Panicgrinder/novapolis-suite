...
def extract_h1(lines: list[str]) -> str | None:
    for ln in lines:
        if ln.startswith("# "):
            return ln[2:].strip()
    return None
...
def derive_category_from_path(md_path: Path) -> str | None:
    rp = relp(md_path)
    # database-rp/<nn>-<name>/...
    parts = rp.split("/")
    if len(parts) < 2:
...
def enrich_pair(md_path: Path, json_path: Path) -> tuple[bool, str]:
    ...
    legacy_chapter = (
        (orig.get("chapter") or "").strip() if isinstance(orig.get("chapter"), str) else ""
    )
    legacy_location = (
        (orig.get("location") or "").strip() if isinstance(orig.get("location"), str) else ""
    )
