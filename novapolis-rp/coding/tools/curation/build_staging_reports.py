...
def find_b_path(a_path: Path) -> Optional[Path]:
...
def find_raw_from_hint(hhmm: Optional[str]) -> Optional[Path]:
...
def extract_hhmm_from_delta_name(delta_out: Path) -> Optional[str]:
...
def regen_for_entry(entry: Dict[str, Any]) -> None:
    ...
    if a_path and raw_path and reports.get('deltaB'):
        ...
        run_py('delta_report.py', [
            str(a_path.relative_to(REPO_ROOT)),
            str(raw_path.relative_to(REPO_ROOT)),
            '--out', str(out_b.relative_to(REPO_ROOT))
        ])
    ...
    if b_path and raw_path and reports.get('deltaC'):
        ...
        run_py('delta_report.py', [
            str(b_path.relative_to(REPO_ROOT)),
            str(raw_path.relative_to(REPO_ROOT)),
            '--out', str(out_c.relative_to(REPO_ROOT))
        ])
