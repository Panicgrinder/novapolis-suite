...
def build_word_safe_regex(token: str, *, alnum_dash_underscore: bool = True, case_sensitive: bool = True) -> re.Pattern:
...
def build_lexicon(yaml_root: Path, extra_aliases: Optional[Dict[str, str]] = None) -> Lexicon:
    by_slug: Dict[str, LexEntry] = {}
    ...
    if "mission-c6-nord" not in by_slug:
        by_slug["mission-c6-nord"] = LexEntry(slug="mission-c6-nord", category="mission", title="Mission C6-Nord (Sealed room)")
...
def detect_fact_candidate(line: str) -> bool:
    ...
    if re.search(rf"\b(\d+[\.,]?\d*)\s*({units})\b", line, flags=re.IGNORECASE):
        return True
    if re.search(r"\b(ist|sind|hat|haben|beträgt)\b.{0,40}\b\d+[\.,]?\d*\b", line, flags=re.IGNORECASE):
        return True
    return False
...
METRO_TERMS = {"metro", "ubahn", "u-bahn", "bahn", "station", "haltestelle", "linie", "strecke", "gleis", "zug", "fahrplan"}
NOTE_PREFIXES = ["Anmerkung:", "Achtung", "Hinweis:", "TODO:", "Todo:", "-", "*", "•", "1)", "1.", "#", ">"]
EVENT_TOKENS = {"ankunft", "abmarsch", "evakuierung", "besprechung", "fund", "unfall", "störung", "start", "stopp", "prüfbericht", "sichtprüfung", "freigabe", "sperre"}
MISSION_TERMS = {"versiegelt", "gesperrt", "quarantäne", "untersuchung", "siegel", "seal", "abgesperrt", "sperrzone", "sperrbereich"}
    ...
    *,
    ...
    try:
        with in_path.open("r", encoding="utf-8", errors="ignore") as fin:
            ...
            for i, line in enumerate(all_lines):
                ...
                for alias, (slug, pat) in alias_patterns.items():
                    if pat.search(line):
                        found_slugs.add(slug)
                ...
                for m in SECTOR_CODE_RE.finditer(line):
                    ...
                    if code not in lex.by_slug:
                        ...
                        lex.by_slug[code] = LexEntry(slug=code, category="location", title=(m.group(1) + m.group(2)).upper())
                        slug_patterns[code] = build_word_safe_regex(code, alnum_dash_underscore=True, case_sensitive=False)
                    found_slugs.add(code)

                # categorize and order
                grouped: Dict[str, List[str]] = {"CHAR": [], "LOC": [], "PROJ": [], "MISSION": [], "ENT": []}
                ...
                if grouped["LOC"] and not (grouped["CHAR"] or grouped["PROJ"] or grouped["MISSION"] or grouped["ENT"]) and not note_flag:
                ...
                if ("c6-nord" in found_slugs) and any(t in "\n".join(all_lines[max(0, i-3):min(len(all_lines), i+4)]).lower() for t in MISSION_TERMS):
    ...
    if locs_per_line:
        ...
        if total >= 10 and hits:
            ...
            if share >= 0.7:
                alias_suggestions.append({"source": "n7", "candidate": cand, "total": total, "hits": count})
...
def load_optional_alias_file(path: Optional[Path]) -> Optional[Dict[str, str]]:
    if not path:
        return None
    try:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap = argparse.ArgumentParser(description="Tag chat chunk files based on YAML front-matter lexicon.")
    ap.add_argument("--yaml-root", default="database-rp", help="Root folder containing Markdown with YAML front-matter")
    ap.add_argument("--chunks-root", required=True, help="Root folder of chunk files 'part-XYZ.txt'")
    ap.add_argument("--out-root", required=True, help="Output root for tagged files and indexes")
    ap.add_argument("--range", required=True, help="Inclusive range like 019-016 (ascending or descending)")
    ap.add_argument("--alias-file", default=None, help="Optional JSON alias file mapping alias->slug")
    ap.add_argument("--dry-run", action="store_true", help="Print per-file summary without writing outputs")
    ...
    ap.add_argument("--retag-in", default=None, help="File or directory with .tagged.txt to re-tag with new heuristics")
    ...
    if args.retag_in:
            ...
            for i, line in enumerate(lines):
                ...
                if N7_RE.search(base_text) and not is_metro_context(lines, i):
                    if "[LOC:c6-nord]" not in base_text:
                        current = base_text + (" " if not base_text.endswith(" ") else "") + "[LOC:c6-nord]"
                    else:
                        current = base_text
                ...
                if ("c6-nord" in base_text.lower() or "[LOC:c6-nord]" in current.lower()) and any(t in window for t in MISSION_TERMS):
        ...
        try:
            unresolved_path = out_root / "unresolved.json"
            unresolved_path.write_text(json.dumps(unresolved_payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        except Exception:
            pass
        # Also dump lexicon for traceability
        try:
            lex_dump = {
                "by_slug": {s: {"category": e.category, "title": e.title} for s, e in lex.by_slug.items()},
                "aliases": lex.aliases,
                "redirects": lex.redirects,
                "deprecated": sorted(list(lex.deprecated)),
            }
            (out_root / "lexicon.json").write_text(json.dumps(lex_dump, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        except Exception:
            pass
        return 0
    ...
    for idx in indices:
        ...
        summary = process_chunk_file(
            in_path, out_path, lex, slug_patterns, alias_patterns, dry_run=args.dry_run, warnings=warnings
        )
    ...
    # Output / Logs
    if args.dry_run:
        print(json.dumps({"summaries": per_file_summaries, **unresolved_payload}, ensure_ascii=False, indent=2))
    else:
        ...
        try:
            ...
            lexicon_dump_path.write_text(json.dumps(lex_dump, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        except Exception as e:
...
""",
        ...
        warnings: List[str] = []
        _ = process_chunk_file(chunk, out, lex, slug_patterns, alias_patterns, dry_run=False, warnings=warnings)
        ...
        warningsm: List[str] = []
        _ = process_chunk_file(chunkm, outm, lex, slug_patterns, alias_patterns, dry_run=False, warnings=warningsm)
        ...
        warnings2: List[str] = []
        _ = process_chunk_file(chunk2, out2, lex, slug_patterns, alias_patterns, dry_run=False, warnings=warnings2)
        ...
        warnings3: List[str] = []
        _ = process_chunk_file(chunk3, out3, lex, slug_patterns, alias_patterns, dry_run=False, warnings=warnings3)
        ...
        warnings4: List[str] = []
        _ = process_chunk_file(chunk4, out4, lex, slug_patterns, alias_patterns, dry_run=False, warnings=warnings4)
