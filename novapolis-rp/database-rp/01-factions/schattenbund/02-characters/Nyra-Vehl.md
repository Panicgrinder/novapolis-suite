---
stand: 2026-03-05 01:00
update: Neuer SSOT-Charakter angelegt (Fraktionsleitung Schattenbund).
checks: scripts/run_checks_and_report.py overall=FAIL; markdownlint=PASS; frontmatter=PASS; path-portability=FAIL; namingpolicy=PASS; todo-index-sync=PASS; doc-freshness=PASS; logs-policy=PASS; ruff=PASS; black=PASS; pytest=PASS; pyright=PASS; mypy=PASS; report=.tmp\results\reports\checks_report_20260305_005843.md
title: Nyra Vehl
category: character
slug: nyra-vehl
version: "0.1"
last_updated: 2026-02-16T17:27:00+01:00
tags: [npc, schattenbund, leitung]
affiliations: [schattenbund]
dependencies: [f9]
primary_location: f9
last_seen: f9
---

<!-- markdownlint-disable MD025 -->

Nyra Vehl
=========

- Rolle: Fraktionsleitung (Schattenbund)
- Status: aktiv
- Kurzprofil: kalkuliert, verdeckt operierend, priorisiert Informationskontrolle und belastbare Loyalitätslinien.

Rollen & Verantwortlichkeiten (Pflichtfelder)
---------------------------------------------

- Fraktionsleitung - verantwortet strategische Ziele, Prioritäten und Eskalationswege des Schattenbunds.
- Steuerung verdeckter Operationen - koordiniert sensible Aufträge, Zugriffsrechte und Risikogrenzen.
- Bündnis-/Konfliktmanagement - trifft finale Entscheidungen bei Verhandlungen und Gegenmaßnahmen.

Zugehörigkeit & Standort
------------------------

- Zugehörigkeit: Schattenbund
- Standort: F9, Führungszelle
- Status: aktiv; Lageabgleich in kurzen Intervallen

Verlinkungen
------------

- Fraktionsübersicht → ../README.md
- Location F9 → ../03-locations/F9.md
- Tick-Regeln (global) → ../../../00-admin/Tick-Regeln-Simulation.md
