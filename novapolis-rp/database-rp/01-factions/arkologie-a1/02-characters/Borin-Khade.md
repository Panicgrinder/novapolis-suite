---
stand: 2026-03-05 01:00
update: Neuer SSOT-Charakter angelegt (Leitung Sicherheit Arkologie A1).
checks: scripts/run_checks_and_report.py overall=FAIL; markdownlint=PASS; frontmatter=PASS; path-portability=FAIL; namingpolicy=PASS; todo-index-sync=PASS; doc-freshness=PASS; logs-policy=PASS; ruff=PASS; black=PASS; pytest=PASS; pyright=PASS; mypy=PASS; report=.tmp\results\reports\checks_report_20260305_005843.md
title: Borin Khade
category: character
slug: borin-khade
version: "0.1"
last_updated: 2026-02-16T17:11:00+01:00
tags: [npc, arkologie-a1, sicherheit]
affiliations: [arkologie-a1]
dependencies: [a1]
primary_location: a1
last_seen: a1
---

<!-- markdownlint-disable MD025 -->

Borin Khade
===========

- Rolle: Leitung Sicherheit (Arkologie A1)
- Status: aktiv
- Kurzprofil: lageorientiert, prozedural streng und auf Prävention statt Eskalation ausgerichtet.

Rollen & Verantwortlichkeiten (Pflichtfelder)
---------------------------------------------

- Sicherheitsleitung - verantwortet Zugangslevel, Schichtprotokolle und Incident-Response der Arkologie A1.
- Freigabekontrolle - koordiniert Sicherheitsfreigaben für Handel, Forschung und externe Kontakte.
- Lageanalyse - bewertet Bedrohungen entlang der Zugänge und passt Schutzmaßnahmen an.

Zugehörigkeit & Standort
------------------------

- Zugehörigkeit: Arkologie A1
- Standort: Arkologie A1 (A1), Sicherheitsleitstand
- Status: aktiv; Schichtführung im Tageszyklus

Beziehungen & Führungseinordnung
--------------------------------

- Führungsbezug: berichtet an die Fraktionsleitung Arkologie A1 (Liora Navesh).
- Schnittstelle Handel: Sicherheitsfreigaben für Tausch- und Transitfenster mit Handelsleitung.

Verlinkungen
------------

- Fraktionsübersicht → ../README.md
- Location A1 → ../03-locations/A1.md
- Tick-Regeln (global) → ../../../00-admin/Tick-Regeln-Simulation.md

