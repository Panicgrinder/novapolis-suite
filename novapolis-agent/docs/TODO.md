...
- [ ] RAG (lokal, optional)
  ...
  - Nächste Schritte:
    - [x] Unit-Tests für Retrieval/Ranking und Save/Load Roundtrip (`retrieve`, `save_index`/`load_index`).
      - Tests: `tests/test_rag_unit.py` deckt Ranking sowie Save/Load Roundtrip ab (Pytest unit marker).
    - [x] Doku: RAG-Nutzung & Indexer (Flags, Pfade, CLI `scripts/rag_indexer.py`, empfohlene Tasks).
      - README: Abschnitt „Lokales RAG“ beschreibt Flags, CLI und Beispielablauf.
