...
update: Wochenarchiv-Snapshot aus dem aktiven Root-Status vor Current-Window-Reduktion erstellt.
...
- 2026-02-23 08:37: Root-Folgepunkte 1-3 aus `todo.root.md` abgeschlossen: neuer VS-Code-Task `Checks: sim epoch assets`, Prioritaetstags `Jetzt/Als naechstes/Spaeter` in Agent/Sim/RP-Boards harmonisiert und verbindliche Wochenabschluss-Routine in `README.md` dokumentiert; `todo.dev.md` und `todo.index.md` entsprechend synchronisiert.
...
- 2026-02-22 12:20: `todo.root.md` inhaltlich konsolidiert. Entfernt/angepasst wurden widersprüchliche Resttexte (u. a. "S5 bleibt offen" trotz `[x]`), historische Planungsmarker klar gekennzeichnet, Duplikatpunkt in "Archivierung" reduziert sowie Steuerzeichen-Artefakte (`U+0011`) bereinigt.
- 2026-02-22 12:04: Etappe 3 (Legacy-Kopfzeilen auslaufen lassen) abgeschlossen. Regel in `.github/instructions/docs-markdown.instructions.md` ergänzt: `Stand:`/`Letzte Aktualisierung:` sind außerhalb YAML-Frontmatter nicht mehr zulässig. `todo.root.md` ist entsprechend auf erledigt gesetzt.
- 2026-02-22 11:14: Etappe-2-Welle 4 (Backups/historische Markdown-Dateien) durchgefuehrt. Ergebnis: in `Backups/**/*.md` und `novapolis-dev/archive/**/*.md` keine case-sensitive Legacy-Kopfzeilen (`Stand:`/`Letzte Aktualisierung:`). Hinweis: Ein einzelner Treffer blieb nur in einer archivierten `.ps1`-Datei (`novapolis-dev/archive/scripts/scripts.ps1-scripts/verify_sim.ps1`) und ist nicht Teil des Markdown-Sweeps. Etappe 2 damit insgesamt abgeschlossen.
- 2026-02-22 11:01: Etappe-2-Welle 3 (`novapolis-rp/database-rp/**` + Rest-`README.md`) durchgefuehrt. Ergebnis: in den README-Dateien keine case-sensitive Legacy-Kopfzeilen; in `database-rp` genau ein Legacy-Treffer (`Stand:`) in `00-admin/canon-canvas.draft.md`, auf `Aktualisiert:` umgestellt. Naechster Arbeitsblock bleibt Welle 4 (Backups-/historische Referenzen nur nach separater Freigabe).
- 2026-02-22 10:57: Etappe-2-Welle 2 (`novapolis-dev/docs/**`, `novapolis_agent/docs/**`) durchgefuehrt. Ergebnis: keine migrierbaren Legacy-Kopfzeilen in `novapolis-dev/docs/**`; in `novapolis_agent/docs/**` ein verbleibender Legacy-Rest (`Letzte Aktualisierung:`) in `reports/overnight-20251022.md` gefunden und auf neutralen Text (`Aktualisiert:`) umgestellt. Naechster Arbeitsblock bleibt Welle 3 (`novapolis-rp/database-rp/**` + Rest-README-Dateien).
- 2026-02-22 10:54: Etappe-2-Welle 1 (Root-Kerndokus) durchgefuehrt. Geprueft wurden `README.md`, `todo.root.md`, `DONELOG.md`, `WORKSPACE_STATUS.md`, `WORKSPACE_INDEX.md`, `PR_DESCRIPTION.md`; Ergebnis: keine migrierbaren Legacy-Kopfzeilen ausserhalb bereits vorhandener YAML-Frontmatter. Naechster Arbeitsblock bleibt Welle 2 (`novapolis-dev/docs/**`, `novapolis_agent/docs/**`).
- 2026-02-22 10:51: S5-Gate aus `todo.root.md` freigegeben. Grundlage: fruehestes Gate-Datum 2026-02-21 wurde erreicht und seit den stabil dokumentierten Laeufen (2026-02-18) liegen >=3 Tage ohne gemeldete Stoerung vor. Ergebnis: Etappe 2 (Snapshot-Frontmatter-Sweep) darf gestartet werden; Etappe 2 selbst bleibt als Arbeitsblock offen bis zur Umsetzung.
