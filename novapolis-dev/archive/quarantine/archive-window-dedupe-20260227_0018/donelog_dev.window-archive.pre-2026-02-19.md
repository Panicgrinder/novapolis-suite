...
update: Doku-Drift-Audit fuer Root/Dev-Dokumente durchgefuehrt; obsolete Pfade in WORKSPACE_INDEX und tests.md korrigiert.
...
- Scope geprueft: Root-Dokus plus Dev-Dokus inklusive TODO-/Index-/Status-Artefakte und `todo.sim.md`.
- Nachweisbare Driftstellen behoben:
  - `WORKSPACE_INDEX.md`: veraltete Eval-Dataset-Verweise entfernt (`chai-ai_small_v1.jsonl`, `gpt_samples.de.jsonl`, `eval-101-300_generated_v1.0.jsonl`, `combined_eval_001-100.jsonl`) und auf aktuelle `neutral/`, `rpg/`, `quality_de_*`-Pakete umgestellt.
  - `novapolis-dev/docs/tests.md`: obsolete Referenz auf `cvn-agent/` durch aktuellen Root-Tasking-Bezug (`.vscode/tasks.json`) ersetzt; Sim-Testabdeckung auf aktuelle Testdateien praezisiert.
- Identifiziert und dokumentiert (noch offen, nicht im selben Lauf bereinigt):
  - `WORKSPACE_STATUS.md` enthaelt umfangreiche historische Bloecke und teils veraltete Zeitanker, die fuer operatives Lesen kuenftig in ein Historik-Artefakt ausgelagert werden sollten.
  - `novapolis-dev/docs/todo.sim.md` enthaelt weiterhin einen expliziten Platzhalterpunkt, der als Backlog-Container bewusst offen bleibt, aber als „placeholder“ markiert ist.
