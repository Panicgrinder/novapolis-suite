...
update: Inhaltliche Konsistenzkorrektur nach Etappe-3-Abschluss (Widersprüche/Altmarker bereinigt).
checks: TODO-Inhalt auf Etappe-2/3-Endstand konsolidiert (S5-Status, Etappe-0-Hinweis, Backout/Planungsmarker), Duplikate reduziert und Steuerzeichen-Artefakte (U+0011) bereinigt (2026-02-22 12:20).
---
...
- Validiert vor Verschiebung:
  ...
  - S5 ist abgeschlossen/freigegeben und bleibt als Historieneintrag dokumentiert.
- [x] S5 - Snapshot-Frontmatter-Migration Etappe 2/3 freigeben:
  - Sweep + Legacy-Auslauf nur nach 3-5 Tagen stabiler Laufpraxis ohne Beschwerden ausführen.
  - Status: freigegeben am 2026-02-22 (Zeit-Gate erreicht; frueheste Freigabe war ab 2026-02-21 moeglich).
  - Freigabeentscheidung 2026-02-22: Seit den stabil dokumentierten Runs vom 2026-02-18 sind >=3 Tage ohne gemeldete Stoerung vergangen; Etappe 2 darf gestartet werden.
...
- [x] Etappe-3-Legacy-Ablösung der Snapshot-Frontmatter-Migration erst nach stabiler Laufpraxis (nur falls Etappe 2 abgeschlossen).
...
- [x] Etappe 0 (2025-11-01 09:10): Historischer Startpunkt (Mischbetrieb erlaubt). Hinweis: Seit Etappe 3 gilt fuer laufende Dokuarbeit nur YAML-Frontmatter; Legacy-Kopfzeilen außerhalb YAML sind nicht mehr zulässig.
...
- [x] Etappe 2: Sweep — bestehende Kopfzeilen migrieren (TODO, README/Index, Policies). Diff klein halten; `checks` kurz.
  - Fortschritt 2026-02-22 (Welle 1 / Root-Kerndokus): `README.md`, `todo.root.md`, `DONELOG.md`, `WORKSPACE_STATUS.md`, `WORKSPACE_INDEX.md`, `PR_DESCRIPTION.md` geprueft; keine migrierbaren Legacy-Kopfzeilen ausserhalb von YAML-Frontmatter gefunden.
  - Fortschritt 2026-02-22 (Welle 2 / Dev+Agent Docs): `novapolis-dev/docs/**/*.md` und `novapolis_agent/docs/**/*.md` gescannt; nur ein Legacy-Rest (`Letzte Aktualisierung:`) in `novapolis_agent/docs/reports/overnight-20251022.md` gefunden und auf neutralen Text (`Aktualisiert:`) umgestellt.
  - Fortschritt 2026-02-22 (Welle 3 / RP-DB + Rest-README): Case-sensitive Sweep in `novapolis-rp/database-rp/**/*.md` und verbleibenden `README.md`-Dateien; genau ein Legacy-Treffer (`Stand:`) in `novapolis-rp/database-rp/00-admin/canon-canvas.draft.md` gefunden und auf `Aktualisiert:` umgestellt.
  - Fortschritt 2026-02-22 (Welle 4 / Backups + historische Markdown-Dateien): Case-sensitive Sweep in `Backups/**/*.md` und `novapolis-dev/archive/**/*.md`; `0` Legacy-Treffer (`Stand:`/`Letzte Aktualisierung:`).
  - Status: Wellen 1-4 abgeschlossen; Etappe 2 erledigt.
- [x] Etappe 3: Legacy-Kopfzeilen auslaufen lassen; Instruktionen aktualisieren (nur YAML erlaubt).
  - Abschluss 2026-02-22: Doku-Instruktion `.github/instructions/docs-markdown.instructions.md` verbietet Legacy-Kopfzeilen (`Stand:`/`Letzte Aktualisierung:`) außerhalb von YAML-Frontmatter.
...
- Backout: Historischer Hinweis aus der Konsolidierungsphase; Etappe 2/3 ist abgeschlossen, Sub-`.vscode`-Rueckfall ist aktuell nicht vorgesehen.

### Betroffene Dateien (historische Planung)
...
- [x] 3-5Tage Nutzung ohne Beschwerden → Go für Etappe2 (erfuellt am 2026-02-22)
