...
try {
    ...
    elseif ($Command) {
        # Wenn das erste Token ein vorhandenes Skript ist, rufe es mit -File auf
        $firstToken = ($Command -split '\\s+', 2)[0].Trim("'\"")
        if ($firstToken -and (Test-Path $firstToken)) {
            $rest = if ($Command -match '\\s+') { ($Command -split '\\s+',2)[1] } else { '' }
            if (Get-Command pwsh -ErrorAction SilentlyContinue) {
                pwsh -NoProfile -ExecutionPolicy Bypass -File $firstToken $rest
                $exit = $LASTEXITCODE
            }
            else {
                powershell -NoProfile -ExecutionPolicy Bypass -File $firstToken $rest
                $exit = $LASTEXITCODE
            }
        }
        else {
