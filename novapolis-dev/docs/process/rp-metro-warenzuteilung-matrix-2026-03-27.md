...
| --- | --- | --- | --- | --- |
| Metro-Rahmen | Novapolis kontrolliert `D5` aktiv, `C6` teilaktiv, `E3` historisch/evakuiert; der Pfad `D5 <-> C6` ist als aktiver Tunnelabschnitt gesetzt. Zusaetzlich ist beim Haendlerbund ein belegter `G7 <-> C6`-Austauschkorridor mit `H-47` vorhanden. | Die Metro-Ebene kann damit evidence-first nur belegte D5/C6-Aufbaupfade, den Haendlerbund-Korridor und die T0-Bandbreiten der uebrigen externen Fraktionen aggregieren, nicht aber freie Gesamtsummen bilden. | Metro-weite Gesamtmengen, neutrale Stationslager und fraktionsuebergreifende Weltgesamtsumme. | `Metrokarte-T0.md`, `Stationskontroll-Matrix.md`, `Warenueberblick-T0.md`, `Haendlerbund-inventar.md` |
...
| Novapolis Tagesdeltas | Fraktionsweit sind fuer Tag 12 -> 13 `1,3 t Baustoffe`, `120 m Schienenprofil`, `18 m2 Betonplatten` und `2` beschaedigte Werkzeuge als Verbrauch/Schaden belegt. | Das Delta taugt als harter Verbrauchsanker und konservativ als `C6-/Nordlinie-Baustellenverbrauch` bei D5-seitiger Quell-/Transferlast. | Restbestandszahlen nach dem Tagesabschluss, konkrete D5-Abbuchung je Posten und konkrete C6-Lager-/Werkstattabbuchung. | `Novapolis-inventar.md`, `Missionslog-Novapolis.md`, `chat-export.normalized.txt` |
| Versorgungslauf `D5 -> C6` | Der Lauf ist als Missionsvorgang und als generische aktive Fracht fuer `Bauteile`, `Werkzeuge` und `Versorgungsgueter` belegt. Prozessanker fuer `Entnahme/Packen`, `Abmeldung` in D5, `manuellerTransport` mit `Tragegestell(ReflexAssist)`, `Ankunft`, `Bestandsaufnahme` und bestaetigten Empfang in C6 liegen vor; Verantwortungsrahmen: Ronja Kerschner fuehrt D5-Abmeldung, Verpacken und Transport, Reflex stuetzt den Lauf. | Konservativ definierbar ist ein missionierter Versorgungslauf aus `D5-Materiallager unter dem Bahnsteig und/oder Werkstattbestand` nach `C6-Empfang` mit anschliessender Baustellenverteilung. | Item-Mengen, Charge, konkreter Quellabgang je Posten sowie die genaue Zielbuchung zwischen Primaer-/Sekundaerlager und Baustellenverteilung. | `Missionslog-Novapolis.md`, `D5-inventar.md`, `C6-inventar.md`, `Novapolis-inventar.md`, `RAW-canvas-2025-10-16T13-05-00-000Z.txt`, `chat-export.txt` |
...
| Haendlerbund (etabliert/external) | T0-Warenbild: Umlaufgueter verfuegbar, stationaere Reserven variabel; zusaetzlich sind `G7` als externer Kontakt-/Umschlagpunkt, `H-47` als dauerhafter Novapolis-Korridor und `C6` als aktivierter Handelsstuetzpunkt belegt. Austauschklassen: `Energie`, `technische Reparaturen`, `Kommunikationszugang` gegen `Nahrungsmittel`, `Filter`, `Grundbedarfsgueter`. | Der Haendlerbund ist damit nicht nur allgemeiner Versorgungsraum, sondern konservativ als konkreter G7-<->-C6-Austauschkorridor lesbar, weiterhin ohne harte Lager- oder Dealmengen. | Konkrete Umlaufmengen, stationaere Reserven, Konvoi-Manifest, Abrechnung und belastbare Bestandsketten je Station. | `Warenueberblick-T0.md`, `Haendlerbund-inventar.md`, `Missionslog-Haendlerbund.md`, `G7.md`, `Fraktionen-Taxonomie.md` |
