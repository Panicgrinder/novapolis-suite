...
update: Der TODO-Index fuehrt nach dem vereinfachten Tree-Policy-Nachzug wieder keine offenen Modul-Boards; Dev, RP, Agent und Sim stehen bei offen: 0.
...
- Dev: `novapolis-dev/docs/todo.dev.md` fuehrt aktuell wieder keine offenen Punkte mehr. Die aktive Tree-Policy zeigt jetzt getrackten Repo-Inhalt in `workspace_tree.txt` und `workspace_tree_dirs.txt` wieder vollstaendig an und blendet nur noch Ignore-basierte Maschinenartefakte plus lokale Repo-Metadatenpfade aus; `novapolis_agent/tests/scripts/test_update_workspace_tree_dirs.py` deckt denselben tracked-only-Schnitt gegen Regression ab.
