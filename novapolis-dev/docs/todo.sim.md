...
update: Der offene Sim-Architekturrest fuehrt jetzt eine dokumentierte Controller-Roadmap fuer alle weiteren sinnvollen Folgeschnitte nach dem AgentRuntimeController.
...
- [ ] [Jetzt] Sim-Hub nach dem Layout-Reset in modulare Controller und Runtime-Dienste zerlegen.
  ...
  - Evidenz: Unter `novapolis-sim/scripts/` liegen mittlerweile bereits eigene Controller fuer Hub-Layout, Hub-Config, Hub-Chat, Session-/Replay-Helper, Session-/Replay-Requests, Session-/Replay-State, Checks/RP, Agent-Studio-UI und jetzt auch Agent-Form-UI. Offen bleibt in `Main.gd` vor allem noch der Agent-Studio-Rest in drei Funktionsgruppen: Payload-Building und Form-Validation (`_build_agent_form_payload_from_controls()`, `_form_control_*()`), Persistenz-/Registry-Schreibpfade (`_apply_*_form_payload()`, `_update_*_registry()`) sowie Runtime-/Prozesssteuerung (`_on_agent_eval_run_pressed()`, `_start_finetune_run()`, `_confirm_destructive_action()`).
