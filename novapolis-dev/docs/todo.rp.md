...
checks: snapshot-lock PASS (2026-06-19 15:40); npx --yes markdownlint-cli2 --config .markdownlint-cli2.jsonc "**/*.md" PASS; .\.venv\Scripts\python.exe scripts\check_frontmatter.py novapolis-dev/docs/todo.rp.md PASS; .\.venv\Scripts\python.exe scripts\check_todo_index_sync.py --repo-root . PASS
---
...
- [ ] [Jetzt] Nordlinie-Folgepaket in drei Zuegen schliessen und Reflex-Herkunft gegen Datenrettung pruefen.
  - Ziel: Die drei aktuell offenen Achsen des aktiven Nordlinie-Slices sollen in drei klar getrennten Folgezuegen auf einen ersten sauberen Curation-/Promotionschnitt gezogen werden: erst die unmittelbare Wirkung des Ronja/Reflex-Moments, dann die konservative Draisine-Entscheidung mit Jonas/Pahl/Lumen, danach Koras/Echos Rueckmeldung nach eigener Sichtpruefung des `Schuttkeil Kontaktseite`. Parallel wird Reflex' Herkunft gegen aktive SSOT und Datenrettungsartefakte geprueft, weil der aktive Kanon derzeit nur eine allgemeine D5-Reaktor-Stabilisierung fuehrt, waehrend die Datenrettung noch Driftlinien wie `Jonas' Werkstatt (Geburtsort Reflex)` traegt.
  - Akzeptanzkriterien:
    1) Zug A spielt zuerst nur die unmittelbare Wirkung des kurzen Ronja/Reflex-Kontakts aus, ohne vorschnell eine endgueltige Bindungs- oder Lore-Behauptung festzuschreiben,
    2) Zug B zieht `Jonas/Pahl/Lumen` plus `Ronja/Reflex` auf eine konservative Erstentscheidung zum Draisine-Antrieb, inklusive verbleibender Blocker zu `Brems-/Stopplogik`, `Not-Aus`, Rueckzugspfad und Lastfenster,
    3) Zug C zieht `Kora/Echo` auf die Eigenpruefung des Schuttkeils und die erneute Funkrueckmeldung an `Ronja/Reflex`, ohne daraus schon freie Material- oder Erfolgsmeldungen zu machen,
    4) vor Promotion oder Finetuning ist explizit festgezogen, welche Punkte danach als bestaetigt kuratiert werden duerfen und welche Restoffenheiten bewusst Runtime-only bleiben,
    5) Reflex' Herkunft ist gegen aktive SSOT, Review-Artefakte und Datenrettungsnotizen geprueft; wenn `unterer Versorgungsschacht von D5` plus Einfluss der dort versiegelten Ronja im aktiven Kanon noch nicht belastbar vorliegt, bleibt dafuer ein sichtbarer Nachzugs-/Korrekturpfad offen statt stiller Ueberschreibung.
  - Evidenz: [../../novapolis-rp/database-curated/staging/rp-runtime/sessions/d5-c6-nordlinie-sanierung-01/scene-log.md](../../novapolis-rp/database-curated/staging/rp-runtime/sessions/d5-c6-nordlinie-sanierung-01/scene-log.md) fuehrt Turn 13 und den Folgeanker mit offenem Ronja/Reflex-Moment, offener konservativer Draisine-Debatte und ausstehender Kora-Rueckmeldung. [../../novapolis-rp/database-rp/01-factions/novapolis/02-characters/Reflex.md](../../novapolis-rp/database-rp/01-factions/novapolis/02-characters/Reflex.md) fuehrt Reflex aktuell nur allgemein als aus der D5-Reaktor-Stabilisierung durch Ronja entstanden; [../../novapolis-rp/database-rp/01-factions/novapolis/Novapolis.md](../../novapolis-rp/database-rp/01-factions/novapolis/Novapolis.md) zieht denselben allgemeinen Schnitt. Die Datenrettungsreview in `novapolis-rp/database-curated/staging/chat-export (1).review.md` fuehrt dagegen noch die Drift `Jonas' Werkstatt (Geburtsort Reflex)` sowie einen `schmalen Schacht`; eine aktive belegte Erfassung des konkreten Ursprungs `unterer Versorgungsschacht von D5` plus Einfluss der dort versiegelten Ronja ist im aktuellen Suchstand nicht sichtbar.
...
- [ ] [Als naechstes] RP-SSOT-to-Runtime-Projektion fuer Governance-Regeln als eigenen Arbeitsstrang schliessen.
