...
@pytest.mark.scripts
@pytest.mark.unit
def test_export_finetune_alpaca_includes_failures(tmp_path: Path, monkeypatch: "pytest.MonkeyPatch") -> None:
    exporter = _load_module()
    ...
    payload = [json.loads(line) for line in produced.read_text(encoding="utf-8").splitlines() if line.strip()]
...
def test_export_finetune_handles_empty_results(tmp_path: Path) -> None:
    ...
    out = asyncio.run(exporter.export_from_results(results_path=str(empty_results), out_dir=str(tmp_path)))
