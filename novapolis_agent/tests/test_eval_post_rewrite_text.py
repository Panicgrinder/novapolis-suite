...
def test_eval_post_rewrite_neutralize_basic() -> None:
    ...
    # Aktiviere Policies via settings mocken? Standardmäßig
    # POLICIES_ENABLED False -> wir erwarten allow.
...
def test_eval_post_rewrite_limits() -> None:
    cm = importlib.import_module("app.core.content_management")
    txt = (
        "Satz eins ist länger als üblich. Satz zwei ist ebenfalls vorhanden! "
        "Satz drei sollte abgeschnitten werden?"
    )
    # Sätze begrenzen
