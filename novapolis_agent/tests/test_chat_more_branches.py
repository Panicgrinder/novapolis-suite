...
@pytest.mark.asyncio
async def test_stream_exception_path_logs_json_and_aborted_memory(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_append_shadow_mode_event_open_failure_branch(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(chat_module.settings, "SHADOW_MODE_LOG_PATH", "tmp/forbidden/shadow.jsonl", raising=False)
    monkeypatch.setattr(chat_module.settings, "SHADOW_MODE_REDACT_PREVIEW_ENABLED", True, raising=False)
...
async def test_process_options_model_dump_non_mapping(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    req = ChatRequest.model_construct(messages=[{"role": "user", "content": "q"}], options=_BadOptions())
