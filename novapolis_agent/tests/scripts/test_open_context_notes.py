...
def test_pick_target_returns_default_for_empty_list() -> None:
    ...
    assert result.endswith(os.path.join("novapolis_agent", "eval", "config", "context.local.md"))
...
def test_open_file_windows_startfile(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        mod.os, "system", lambda cmd: (_ for _ in ()).throw(RuntimeError("should not call"))
    )
...
@pytest.mark.scripts
@pytest.mark.unit
def test_ensure_file_recovers_when_sample_read_fails(
