...
def test_export_finetune_no_rows_returns_error(tmp_path: Path) -> None:
    ...
    out = asyncio.run(
        exporter.export_from_results(
            ...
            patterns=[str(tmp_path / "none.jsonl")],
        )
    )
...
@pytest.mark.scripts
@pytest.mark.unit
def test_rerun_failed_missing_items_in_datasets(
    ...
    with open(res, "w", encoding="utf-8") as f:
        ...
        f.write(
            json.dumps({"item_id": "eval-404", "success": False, "failed_checks": ["x"]}) + "\n"
        )

    # DATASETS_DIR leer lassen -> keine Zuordnung
    monkeypatch.setattr(mod, "RESULTS_DIR", os.fspath(results_dir), raising=False)
    monkeypatch.setattr(
        mod, "DATASETS_DIR", os.fspath(tmp_path / "eval" / "datasets"), raising=False
    )
