...
def test_session_memory_returns_copy() -> None:
    ...
    memory.put_and_trim(
        "sid", [{"role": "user", "content": "eins"}], max_messages=10, max_chars=100
    )
...
def test_session_memory_trims_messages_and_characters() -> None:
    ...
    memory.put_and_trim(
        "sid", [{"role": "assistant", "content": "zwei"}], max_messages=2, max_chars=50
    )
    ...
    memory.put_and_trim(
        "sid", [{"role": "assistant", "content": "x" * 32}], max_messages=5, max_chars=10
    )
