#!/usr/bin/env python3
"""Build staging artifacts for pending RAW chat exports.

- Reads novapolis-rp/database-curated/staging/manifest.json
- Filters entries with id starting 'pending-raw-chat-export' and type 'raw-txt'
...
def main() -> int:
    ...
    for entry in data:
        ...
        if not (
            entry_id.startswith("pending-raw-chat-export") or entry_id == "export-3-chat-export-0"
        ):
            continue
    ...
    print(f"OK: Chat-RAW-Staging aktualisiert (entries: {processed})")
    return 0
