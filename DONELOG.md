...
- 2026-03-11 01:17: Sim-Hub-Layout nach Screenshot-Befund nachgezogen. In `novapolis-sim/Main.tscn` wurde das aktive Hintergrundbild auf die neueste Asset-Datei umgestellt (`res://assets/ChatGPT Image 11. März 2026, 01_12_53.png`) und der Hintergrund auf volle Viewport-Flaeche gesetzt (Offsets auf `0`). In `novapolis-sim/scripts/Main.gd` wurde `_apply_editor_hub_layout` auf die vorbereiteten Felder `bereich-01..04` ausgerichtet (Top-Infos, Aktionsbuttons, Server-/Fehlerblock, Config-Panel), inklusive sauberem Innenlayout des Chat-Panels (`History`, `Input`, `Senden`, Status), damit Texte/Schaltflaechen/Textfelder innerhalb der vorgesehenen Bereiche konsistent sitzen.

- 2026-03-11 00:26: Hotfix fuer Godot-Parsefehler in `novapolis-sim/scripts/Main.gd` umgesetzt. Ursache war ein Tippfehler bei der Identifier-Referenz (`_hub_chat_history_label` statt deklarierter `hub_chat_history_label`) im Initialisierungsblock; die Referenz wurde auf den vorhandenen `@onready`-Bezeichner korrigiert, wodurch der LSP-Parsefehler in `Main.gd` behoben ist.
...
- 2026-03-04 00:14: Der verbleibende RP-Panel-Restpunkt wurde dokumentarisch abgeschlossen, da die technische Umsetzung bereits im Code vorhanden ist (`Hour +1`, `Auto-Advance`, `Replay-Seed` samt `RP_*`-Events in `novapolis-sim/scripts/Main.gd`). Board-Sync im selben Lauf: `novapolis-dev/docs/todo.sim.md` aktualisiert, `novapolis-dev/docs/todo.index.md` auf `offen: 2` reduziert (`Sim v4.6`), `novapolis-dev/docs/donelog.md` nachgezogen.

- 2026-03-04 00:14: Verbleibende Agent-Studio-Menuepunkte in `novapolis-sim/scripts/Main.gd` umgesetzt. Neu sind zusammengefuehrte Statuslinien fuer `Artifacts`, `Experiments`, `Policy Sandbox`, `Release Gate`, `Audit Trail` und `Security`. Runtime-Events werden zusaetzlich persistent in `user://agent_user_data/audit/trail.jsonl` protokolliert; fuer destruktive Stop-Aktionen gilt jetzt ein Zwei-Schritt-Guard (`Eval/Datasets/Finetune Stop`) mit konfigurierbarem Security-Status unter `user://agent_user_data/security/model.json`. Doku-Sync im selben Lauf: `novapolis-dev/docs/todo.sim.md` (sechs Punkte auf erledigt), `novapolis-dev/docs/todo.index.md` (`offen: 3`, `Sim v4.5`) und `novapolis-dev/docs/donelog.md`.

- 2026-03-04 00:09: Weitere Sim-Restpunkte in `novapolis-sim/scripts/Main.gd` umgesetzt. `Synonyms` kann nun optional per Importpfad laden und per Exportpfad schreiben; zusaetzlich werden Delta (`+terms/+syns`) und Validatorstatus (`ok|warn`) im Status ausgewiesen. Der Finetune-Status zeigt jetzt Laufmetriken (`epochs`, `max_steps`, `batch_size`, `lr`) in Running/Done/Failed. Die Eval-Zusammenfassung wurde um eine kompakte Trendkarte erweitert (`pass`, `delta`, `regress`, `drift`, `avg_ms`) auf Basis der letzten Runs. Dokument-Sync im selben Lauf: `novapolis-dev/docs/todo.sim.md` (`Synonyms`/`Finetuning`/`KI-Stand` erledigt), `novapolis-dev/docs/todo.index.md` (`offen: 9`, `Sim v4.4`) und `novapolis-dev/docs/donelog.md`.

- 2026-03-04 00:04: `novapolis-sim/scripts/Main.gd` im Agent-Studio-Jobsflow erweitert. Der Form-Target-Selector unterstuetzt jetzt `new`, `retry_latest` und `cancel_latest`; Retry reiht den letzten `failed/cancelled` Job erneut ein, Cancel markiert den letzten `queued/running` Job als `cancelled`. Zusaetzlich wurden Queue-Helfer fuer Laden/Schreiben/Statusaggregation eingefuehrt und die Jobs-Statuszeile auf aggregierte Zustandszahlen erweitert. Doku-Sync im selben Lauf: `novapolis-dev/docs/todo.sim.md` (`Jobs` erledigt), `novapolis-dev/docs/todo.index.md` (`offen: 12`, `Sim v4.3`) und `novapolis-dev/docs/donelog.md`.

- 2026-03-03 23:43: Sim-TODO-Weiterlauf in `novapolis-dev/docs/todo.sim.md` fortgesetzt. Drei offene Agent-Studio-Punkte wurden auf Basis bestehender Implementierung geschlossen: `Datasets` (Source + realer Curation-Run + Registry-Active), `Profiles` (Form + Persistenz + Active/Archive-Registry) und `Advanced Settings` (Form + Persistenz + Statusanzeige). Dokument-Sync im selben Lauf nachgezogen: `novapolis-dev/docs/todo.index.md` (`offen: 13`, Hinweis `Sim v4.2`) und `novapolis-dev/docs/donelog.md`.

- 2026-03-03 23:39: Offener Menuepunkt `RP/Content` aus `todo.sim` technisch abgeschlossen. `novapolis-sim/scripts/Main.gd` wurde um `_rp_content_summary` erweitert; die Hub-Event-Karte zeigt jetzt `module=rp`, `vis=...`, `src=<epoch/pc_log@slot>` und das letzte `RP_*`-Event.

- 2026-03-03 23:34: Offener Sim-Statuspunkt `Qualitaet` umgesetzt. In `novapolis-sim/scripts/Main.gd` liest `_refresh_quality_status` den neuesten Report unter `.tmp/results/reports/checks_report_*.json` und schreibt `tests_last`, kombinierte `types_last`-Bewertung (`pyright/mypy`) sowie `coverage_last` in die Hub-Eval-Karte (`eval_card_notes_label`).

- 2026-03-03 23:22: Restpunkte aus `novapolis-dev/docs/todo.sim.md` technisch umgesetzt. In `novapolis-sim/scripts/Main.gd` wurde `event_rate` als gleitende 30s-Rate im Queue-Status integriert (`hub_queue_label`, `sim_card_queue_label`), der aktive `dataset_tag` im Artefaktstatus der Eval-Karte nachgezogen (`eval_card_artifacts_label`) und ein `last_error_code` aus Statusmeldungen extrahiert/angezeigt (`hub_errors_label`, Helper `_extract_error_code`).
...
- 2026-02-27 10:57: Quellenklarheit gegen Regelverwechslung gehaertet. In `.github/copilot-instructions.md` wurde unter `Dateipfad & Geltungsbereich` ein bindender Abschnitt `Aktive vs. sekundäre Quellen` ergaenzt (aktiv nur `.github/copilot-instructions.md` + `.github/instructions/*.instructions.md`; `novapolis-dev/docs/copilot-vscode-usage.md` nur Guidance; `novapolis-dev/archive/**` nie aktive Regelbasis). Der Navigator-Agent (`.github/agents/novapolis-workspace-navigator.agent.md`) wurde entsprechend erweitert. In `novapolis-dev/docs/copilot-vscode-usage.md` wurde der Nicht-Bindungsstatus explizit gemacht und das `applyTo`-Snippet als Illustration markiert. Archivierte Instruction-Kopien (`novapolis-dev/archive/docs/others/copilot-instructions.2025-11-15 23-48.md`, `novapolis-dev/archive/docs/others/copilot-instructions-headings.archive.md`) erhielten einen sichtbaren `Archivhinweis (nicht bindend)`. Wegen R-IDX wurde `.github/copilot-instructions-headings.md` im selben Lauf synchronisiert. Zusatzfix: `scripts/check_frontmatter.py` ignoriert nun `.github/agents/`, damit das VS-Code-Agent-Schema (ohne `stand/update/checks`) nicht im Widerspruch zum Validator steht. Checks: markdownlint-cli2 PASS; check_frontmatter.py PASS.

- 2026-02-27 10:47: Governance-Navigation und Gate-Abgleich geschaerft. `.github/copilot-instructions.md` erhielt einen kompakten `TL;DR / Runtime Essentials`-Block, einen verbindlichen Regel-ID-Index (`ID | Kurzname | Abschnitt`) sowie feste Landepunkte pro Kernregel (`#### R-*`). Gleichzeitig wurde die Scoped-Liste in der Kern-SSOT auf `mind-cluster.instructions.md` synchronisiert; `.github/copilot-instructions-headings.md` wurde wegen R-IDX im selben Lauf aktualisiert. CI-Abgleich: In `.github/workflows/markdownlint.yml` wurde der Frontmatter-Validator-Scope auf aktive Governance-/Dokupfade erweitert (`.github/instructions`, `.github/copilot-instructions-headings.md`, `novapolis-dev/docs`, `novapolis_agent/docs`, `README.md`, `DONELOG.md`, `todo.root.md`, `WORKSPACE_STATUS.md`, `WORKSPACE_INDEX.md`). Checks: markdownlint-cli2 PASS; check_frontmatter.py PASS.
...
- 2026-02-23 22:27: In `.github/instructions/rp-docs.instructions.md` die TODO-Governance verschaerft: neue Pflichtregel `R-TODO-IDX` erfordert bei jeder Aenderung an einer TODO-Datei im Scope die Aktualisierung von `novapolis-dev/docs/todo.index.md` im selben Aenderungslauf.
...
- 2026-02-22 16:27: README-Prüfpunkt vor Archivierung erneut voll geprüft (neuer Voll-Rescan: `.tmp/results/reports/readme_full_rescan_20260222_1627.md`). Ergebnis: 73/73, `missing_link_count=0`, `flag_count=0`. Anschließend den abgeschlossenen README-Block aus `todo.root.md` nach `novapolis-dev/archive/todo.root.archive.md` verschoben.

- 2026-02-22 16:17: README-Gesamtlauf abgeschlossen. Die verbliebenen 57 Dateien wurden in einem Einmal-Durchlauf automatisiert verifiziert (`.tmp/results/reports/readme_remaining_scan_20260222_1616.md`), Ergebnis: keine fehlenden relativen Linkziele, keine neuen Driftmuster. Gesamtstand damit 73/73 geprüft.

- 2026-02-22 16:06: Weiterer 4er-Batch kompakter README-Reviews abgeschlossen (`novapolis-rp/database-rp/01-factions/novapolis/README.md`, `00-doctrine/README.md`, `02-characters/README.md`, `03-locations/README.md`). Alle Links/Zielpfade verifiziert, kein neuer Driftpunkt.

- 2026-02-22 16:00: Nächster 4er-Batch kompakter README-Reviews abgeschlossen (`novapolis-dev/archive/README.md`, `novapolis-rp/database-raw/99-exports/README.md`, `novapolis-rp/database-curated/final/README.md`, `novapolis-rp/database-curated/staging/README.md`). Alle Referenzen/Pfade verifiziert; kein neuer Driftpunkt.

- 2026-02-22 15:47: Nacharbeiten aus dem 4er-README-Batch abgeschlossen. `novapolis-rp/README.md` wurde vom veralteten Task-Verweis bereinigt; `packages/README.md` beschreibt die Shared-Paket-Installation nun korrekt als expliziten Editable-Install bei tools-only Root-`pyproject.toml`.

- 2026-02-22 15:44: 4er-Batch kompakter README-Reviews abgeschlossen (`novapolis-rp/README.md`, `novapolis-dev/README.md`, `novapolis_agent/eval/README.md`, `packages/README.md`). Zwei Driftpunkte identifiziert: veraltete Task-Referenz in `novapolis-rp/README.md` sowie veraltete Editable-Install-Aussage in `packages/README.md`.

- 2026-02-22 15:37: `novapolis-sim/README.md` korrigiert. Der veraltete Verweis auf den nicht vorhandenen Task `Run Agent Dev` wurde entfernt und durch den gültigen direkten Uvicorn-Startweg für `app.api.sim:app` ersetzt.

- 2026-02-22 15:31: Vierte README-Einzelprüfung durchgeführt (`novapolis-sim/README.md`, 64 Zeilen). Kernclaims (Projektdatei, Szene, Verifier, SimClient, Archivpfad) verifiziert. Befund: Task-Referenz `Run Agent Dev` ist veraltet, da in `/.vscode/tasks.json` nicht vorhanden.

- 2026-02-22 15:26: Root-README korrigiert. Der Abschnitt "Aktuelle Statusdokumente" wurde von veralteten 2025-Datumsständen auf 2026-konsistente, nicht-statisch datierte Formulierungen umgestellt.

- 2026-02-22 15:23: Dritte README-Einzelprüfung durchgeführt (`README.md` im Root, 94 Zeilen). Kernclaims verifiziert (Pfade/Skripte/Workflow-Topologie vorhanden). Festgestellter Driftpunkt: veraltete Datumsangaben im Abschnitt "Aktuelle Statusdokumente" (2025-11), obwohl laufende Pflege 2026 dokumentiert ist.

- 2026-02-22 15:19: Zweite README-Einzelprüfung durchgeführt (`Backups/README.md`, 102 Zeilen). Inhalt gegen Ist-Stand verifiziert: Manifest-/Rotation-Skripte und Backup-Artefakte (`manifest.v1.json`, `manifest.v1.sha256sum.txt`, `rotation.log`) vorhanden; keine kritische Inhaltsdrift festgestellt.

- 2026-02-22 15:04: Einzelprüfung `novapolis_agent/README.md` (umfangreich, 300 Zeilen) durchgeführt. Befunde: veralteter Verweis auf Task `Run Agent Dev`, nicht belegter CI-Reports-Claim (kein entsprechender Workflow in `.github/workflows/`), veralteter Verweis auf `pyrightconfig.scripts.json` (Datei fehlt). Evidenz im Root-TODO unter Punkt 1 ergänzt.

- 2026-02-22 14:53: Root-Backlog geschärft: Punkt 1 wurde von einer reinen Root-README-Aufgabe auf eine workspace-weite README-Inhaltsprüfung erweitert. Umfangreiche READMEs sind explizit einzeln zu prüfen, nicht im Batch.

- 2026-02-22 14:41: TODO-Aktualitätsabgleich umgesetzt in der gewünschten Reihenfolge (2 -> 1 -> 3). In `novapolis-dev/docs/todo.sim.md` wurde der Headless-Lade-Check auf erledigt gesetzt (mit Evidenzverweis auf `WORKSPACE_STATUS.md` und `novapolis-dev/docs/donelog.md`). Danach wurden in `novapolis-dev/docs/todo.index.md` die Open-Counts auf den Ist-Stand korrigiert (RP: 20, SIM: 7).

- 2026-02-22 14:17: Voller Test-/Checks-Block und Doku-Refresh durchgeführt. Läufe: Root-Pytest-Wrapper, `pytest -q`, `pytest -q -m unit`, `pytest -q -m "api or streaming"`, Coverage-Wrapper, `run_checks_and_report.py` (nach Black-Fix in `scripts/scan_legacy_markdown_headers.py` vollständig PASS). Tree-Dokus neu erzeugt: `workspace_tree_full.txt`, `workspace_tree.txt`, `workspace_tree_dirs.txt`.

- 2026-02-22 14:01: `todo.root.md` mit konkreten neuen Root-Aufgaben befuellt. Aufgenommen wurden: README-Statusharmonisierung (2026), Quarantaene-Workflow fuer leere Dateien/Ordner (WhatIf vor Apply), woechentlicher Root-Qualitaetslauf, konkrete Top-3-Backlogpunkte (Agent/RP/Dev) und CI-Entscheidung zu Doku-Gates fuer Branch-Pushes.

- 2026-02-22 12:35: Root-TODO archiviert und neu gestartet. Vollinhalt aus `todo.root.md` als Snapshot nach `novapolis-dev/archive/quarantine/todo-root-snapshot-20260222_1234.md` kopiert, Referenz in `novapolis-dev/archive/todo.root.archive.md` ergänzt und `todo.root.md` auf schlanke aktive Vorlage für neue Punkte zurückgesetzt.

- 2026-02-22 12:20: `todo.root.md` inhaltlich nachgezogen. Widersprüche nach Etappe-3-Abschluss bereinigt (u. a. S5-Status im Später-Block), historische Marker präzisiert (`Betroffene Dateien (historische Planung)`, Backout-Hinweis), Duplikatpunkt entfernt und Steuerzeichen-Artefakte (`U+0011`) in Textstellen wie "Sprint 1"/"Phase 0/1" korrigiert.

- 2026-02-22 12:04: Snapshot-Frontmatter-Migration Etappe 3 abgeschlossen. In `.github/instructions/docs-markdown.instructions.md` wurde die Regel ergänzt, dass `Stand:`/`Letzte Aktualisierung:` außerhalb YAML-Frontmatter nicht zulässig sind. Offene Etappe-3-Punkte in `todo.root.md` geschlossen.

- 2026-02-22 11:14: Snapshot-Frontmatter-Migration Etappe 2 final abgeschlossen (Welle 4). Case-sensitive Sweep in `Backups/**/*.md` und `novapolis-dev/archive/**/*.md` ergab `0` Legacy-Kopfzeilen (`Stand:`/`Letzte Aktualisierung:`). Ein einzelner `Stand:`-Treffer in einer archivierten `.ps1`-Datei wurde bewusst nicht angepasst (außerhalb des Markdown-Sweeps).

- 2026-02-22 11:11: Folgefix fuer Shell-Integration. In `.vscode/tasks.json` wurden die drei aktiven `pwsh`-Tasks (`Workspace tree: full`, `Workspace tree: directories`, `TTS: export`) von `-NoProfile` auf `-NoLogo` umgestellt, damit keine widerspruechliche Terminal-Startkonfiguration mehr im aktiven Workspace verbleibt.

- 2026-02-22 11:06: Shell-Integration nachgezogen. In `.vscode/settings.json` wurden fuer `terminal.integrated.profiles.windows.PowerShell 7.args` und `terminal.integrated.automationProfile.windows.args` die Argumente von `-NoProfile` auf `-NoLogo` umgestellt, damit die VS-Code-Shell-Integration stabil laden kann.

- 2026-02-22 11:01: Snapshot-Frontmatter-Migration Etappe 2 fortgesetzt (Welle 3 abgeschlossen). Case-sensitive Sweep in `novapolis-rp/database-rp/**/*.md` und verbleibenden `README.md`-Dateien: kein README-Legacy-Treffer, genau ein RP-Treffer (`Stand:`) in `novapolis-rp/database-rp/00-admin/canon-canvas.draft.md`, auf `Aktualisiert:` umgestellt. Etappe 2 bleibt insgesamt offen (Welle 4 ausstehend).

- 2026-02-22 10:57: Snapshot-Frontmatter-Migration Etappe 2 fortgesetzt (Welle 2 abgeschlossen). `novapolis-dev/docs/**` und `novapolis_agent/docs/**` gescannt; einzig verbleibender Legacy-Rest (`Letzte Aktualisierung:`) in `novapolis_agent/docs/reports/overnight-20251022.md` auf neutralen Text (`Aktualisiert:`) umgestellt. Etappe 2 bleibt insgesamt offen (Wellen 3-4 ausstehend).

- 2026-02-22 10:54: Snapshot-Frontmatter-Migration Etappe 2 gestartet (Welle 1 abgeschlossen). Root-Kerndokus (`README.md`, `todo.root.md`, `DONELOG.md`, `WORKSPACE_STATUS.md`, `WORKSPACE_INDEX.md`, `PR_DESCRIPTION.md`) geprueft; keine migrierbaren Legacy-Kopfzeilen ausserhalb bereits vorhandener YAML-Frontmatter gefunden. Folge: Etappe 2 bleibt insgesamt offen, naechster Block ist Welle 2 (`novapolis-dev/docs/**`, `novapolis_agent/docs/**`).

- 2026-02-22 09:25: Terminal-Startup stabilisiert. In `.vscode/settings.json` wurde `task.allowAutomaticTasks` auf `off` gesetzt und `terminal.integrated.enablePersistentSessions` auf `false`, damit beim VS-Code-Start keine alten/mehreren Terminal-Sessions wiederhergestellt oder durch Auto-Tasks zusaetzlich gestartet werden.
- 2026-02-22 08:10: GitLens-Bereinigung abgeschlossen. `eamodio.gitlens` aus `.vscode/extensions.json` entfernt (Workspace empfiehlt die Extension nicht mehr), danach Uninstall in VS Code und VS Code Insiders per CLI ausgefuehrt.
- 2026-02-22 07:44: Workspace-Extension-Empfehlung bereinigt, damit unter VS Code Insiders keine Copilot-Install-Aufforderung mehr erzwungen wird. In `.vscode/extensions.json` wurde `github.copilot` aus `recommendations` entfernt und unter `unwantedRecommendations` aufgenommen; `github.copilot-chat` bleibt unveraendert empfohlen.
- 2026-02-22 07:40: CI-Fix fuer ausgefallene GitHub-Checks umgesetzt. In `.github/workflows/ci.yml` wurden die Install-Pfade auf `requirements/base.txt` und `requirements/dev.txt` korrigiert (Job `python-ci`, working-directory `novapolis_agent`). In `.github/workflows/markdownlint.yml` wurde `single-root-todo.md` aus dem Frontmatter-Validator-Scope entfernt, da die Datei nicht mehr existiert. Ziel: `python-ci` und `frontmatter (validator)` wieder gruen.
...
- 2026-02-21 04:28: Root-TODO-Archivierung weitergefuehrt. Vollstaendig abgeschlossenen Block `Lokale AI - Zusammenfassung (Checkliste)` aus `todo.root.md` in `novapolis-dev/archive/todo.root.archive.md` ueberfuehrt; im aktiven Backlog durch kompakten Validierungsverweis ersetzt. Scoped Checks: markdownlint + frontmatter PASS.
- 2026-02-21 04:26: Doku-Sync vor Weiterarbeit umgesetzt. `single-root-todo.md` wurde nach `novapolis-dev/archive/quarantine/single-root-todo.md` verschoben; aktive Referenzen in `todo.root.md` und `novapolis-dev/archive/todo.root.archive.md` nachgezogen. Zudem abgeschlossene Bloecke aus `todo.root.md` in `novapolis-dev/archive/todo.root.archive.md` ueberfuehrt (Wrapper-Migration, Priorisierung->Jetzt), jeweils mit Validierung (markdownlint/frontmatter scoped PASS).
- 2026-02-21 04:14: Commit/Pub erfolgreich (`fa62443`) mit Dokumentpflege-Delta: Agent-TODO auf `novapolis-dev/docs/todo.agent-board.md` umgestellt, `novapolis-dev/docs/todo.agent.md` entfernt, Root-Archivierung konsolidiert, `single-root-todo.md` aus Root entfernt und unter Quarantine abgelegt.
